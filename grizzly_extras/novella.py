...
class MarkdownAstType(Enum):
    ...
    @classmethod
    def from_value(cls, value: str | None) -> MarkdownAstType:
...
class MarkdownAstNode:
    ...
    _first_child: MarkdownAstNode | None = field(init=False, default=None)
    ...
    def raw(self) -> str | None:
        return cast('str | None', self.ast.get('raw', None))
...
def _create_nav_node(target: list[str | dict[str, str]], path: str, node: Path, *, with_index: bool = True) -> None:
...
def mkdocs_update_config(config: dict[str, Any]) -> None:  # pragma: no cover
    ...
    nav_testdata: list[str | dict[str, str]] = []
    ...
    nav_users: list[str | dict[str, str]] = []
    ...
    nav_steps_background: list[str | dict[str, str]] = []
    ...
    nav_steps_scenario: list[str | dict[str, str]] = []
    ...
    nav_steps_scenario_tasks: list[str | dict[str, str]] = []
...
class GrizzlyMarkdownInlineParser(InlineParser):
    def parse_codespan(self, match: Match, state: InlineState) -> int:
        ...
        pattern = re.compile(r'(.*?[^`])' + marker + r'(?!`)', re.DOTALL)
...
class GrizzlyMarkdown:
    ...
    _index: int
    ignore_until: Callable[[MarkdownAstNode], bool] | None
    offset: int
    debug: bool
    ...
    def get_code_block(self, start_node: MarkdownAstNode) -> str | None:
    ...
    @classmethod
    def get_step_expression_from_code_block(cls, code_block: str) -> tuple[str, str] | None:
    ...
    def _process_content(self, content: str) -> str:  # noqa: C901, PLR0915, PLR0912
        ...
        last_node: MarkdownAstNode | None = None

        for index, node_ast in enumerate(reversed(self._ast_tree_modified)):
            node = MarkdownAstNode(node_ast, -index)

            if node.type != MarkdownAstType.BLANK_LINE and not (
                node.type == MarkdownAstType.PARAGRAPH and node.first_child.type == MarkdownAstType.INLINE_HTML and self._is_anchor(node.first_child.raw or '')
            ):
                last_node = node
                break
        ...
        return renderer(self._ast_tree_modified, state=BlockState())
