...
def test_after_scenario(behave_fixture: BehaveFixture) -> None:
    ...
    grizzly.scenario.tmp_tasks.async_group = AsyncRequestGroupTask(name='test-async-1')
    ...
    grizzly.scenario.tmp_tasks.timers['test-timer-1'] = TimerTask('test-timer-1')
    grizzly.scenario.tmp_tasks.timers['test-timer-2'] = TimerTask('test-timer-2')

    with pytest.raises(AssertionError) as ae:
        after_scenario(behave)
    assert str(ae.value) == 'timers test-timer-1, test-timer-2 has not been closed'

    grizzly.scenario.tmp_tasks.timers.clear()
