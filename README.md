...
```bash
# if there is only one feature file in features/
grizzly-cli --local

# if there is multiple features files
grizzly-cli --local features/test.feature
```

Distributed (requires `docker` and `docker-compose` where the command is executed):
...
# if there is multiple features files
grizzly-cli features/test.feature

# rebuild container image when there has been changes
grizzly-cli --force-build features/test.feature
```
